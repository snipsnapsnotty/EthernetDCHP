...
DhcpMessageType_t EthernetDHCPClass::_processDhcpReply()
{
   ...
   *((uint16_t*)&udp_len) = ethutil_ntohs(*((uint32_t*)(buf+6)));
   ...
   ethernet_compat_read_data(this->_socket, (uint16_t)ptr, (uint16_t)bootpPacket, sizeof(BootpPacket_t));
   ...
   if (BootpReply == bootpPacket->opCode && DHCP_SERVER_PORT == svr_port &&
       ...
       ethutil_ntohl(bootpPacket->xid) == this->_dhcpData.xid) {
         ...
         ethernet_compat_read_data(this->_socket, (uint16_t)ptr + 236, (uint16_t)buf, sizeof(uint32_t));
         if (0 == memcmp(buf, &magicCookie, sizeof(uint32_t))) {
            ...
            ethernet_compat_read_data(this->_socket, (uint16_t)ptr + 240, (uint16_t)buf, opt_len);
